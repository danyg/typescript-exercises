...
function requestCurrentServerTime(callback: (response: NumberApiResponse) => void) {
	callback({
		status: 'success',
		data: Date.now()
	});
...
function requestCoffeeMachineQueueLength(callback: (response: NumberApiResponse) => void) {
	callback({
		status: 'error',
