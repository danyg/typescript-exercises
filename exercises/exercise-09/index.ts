...
type PromisifiedF<T> = () => Promise<T>;
type FtoPromisify<T> = (callback: (response: ApiResponse<T>) => void) => void;

function promisify<T>(arg: FtoPromisify<T>): (PromisifiedF<T>) {
	return () => new Promise(
		(resolve, reject) => arg((response: ApiResponse<T>) =>
			'error' in response ?
